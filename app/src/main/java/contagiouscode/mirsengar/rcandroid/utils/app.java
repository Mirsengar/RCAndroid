...
void closeKeyboard ( View view ) {
     ...
     if ( view != null ) {
          InputMethodManager imm =
                    ( InputMethodManager ) Application.getContext ( ).getSystemService ( Context.INPUT_METHOD_SERVICE );
...
public static
boolean isProbablyArabic ( String s ) {
...
public static
void changeStatusBarColor ( Activity activity , Boolean white ) {

  /*      View decor = activity.getWindow().getDecorView();
        if (Build.VERSION.SDK_INT >= 21) {
            activity.getWindow().setNavigationBarColor(ContextCompat.getColor(activity,
                    R.color.colorStatusBarFooter
            )); // Navigation bar the soft bottom of some phones like nexus and some Samsung note
             series
...
*/
     ...
     View decor = activity.getWindow ( ).getDecorView ( );
     if ( Build.VERSION.SDK_INT >= 21 ) {
          activity.getWindow ( ).setNavigationBarColor ( ContextCompat.getColor (
                    activity ,
...
//        activity.getResources().updateConfiguration(config,activity.getResources()
//        .getDisplayMetrics());
