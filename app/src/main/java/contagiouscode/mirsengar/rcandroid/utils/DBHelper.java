...
public
boolean insertPeople ( List < PeopleObject > list ) {
...
public
Cursor getPeopleByID ( String ID ) {
...
public
Cursor searchPeople ( String query ) {
     SQLiteDatabase db = this.getReadableDatabase ( );
     ...
     Cursor c = db.rawQuery (
               "select * from people where ID LIKE '%" + query + "%' OR fName LIKE '%" + query +
               "%' OR lName LIKE '%" + query + "%' OR plaque LIKE '%" + query + "%'"
               , null );
