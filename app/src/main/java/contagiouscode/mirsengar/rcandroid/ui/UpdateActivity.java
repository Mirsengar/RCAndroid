package contagiouscode.mirsengar.rcandroid.ui;
...
import ir.ashkaran.mzq.api.RCApi;
...
public
class UpdateActivity extends BaseActivity {
DBHelper database;
...
void getData ( ) {
     ...
     UserModel                  userModel = new Gson ( ).fromJson ( Storage.get ( "UserModel" ) ,
                                                                    UserModel.class );
     HashMap < String, String > body      = new HashMap <> ( );
     body.put ( "token" , Storage.get ( "token" ) );
     body.put ( "city" , userModel.getCity ( ) );
     Call < List < PeopleObject > > call =
               Application.getRetrofit ( ).create ( MzQAPI.class ).GetData ( body );
     call.enqueue ( new Callback < List < PeopleObject > > ( ) {
          @Override
          public
          void onResponse ( Call < List < PeopleObject > > call ,
                            Response < List < PeopleObject > > response ) {
          ...
          @Override
          public
          void onFailure ( Call < List < PeopleObject > > call , Throwable t ) {
...
void uploadCachedCommuting ( ) {
     ...
     CachedCommutingObject      object = new CachedCommutingObject ( Storage.get ( "token" ) ,
                                                                     cached );
     Call < MainResponseModel > call   =
               Application.getRetrofit ( ).create ( RCApi.class ).InsertCommuting ( object );
     
     showProgress ( "در حال ارسال اطلاعات ذخیره شده در دستگاه" );
     call.enqueue ( new Callback < MainResponseModel > ( ) {
          @Override
          public
          void onResponse ( Call < MainResponseModel > call ,
                            Response < MainResponseModel > response ) {
               if ( response.isSuccessful ( ) ) {
                    Cache.ClearCache ( );
          ...
          @Override
          public
          void onFailure ( Call < MainResponseModel > call , Throwable t ) {
               getData ( );
               showProgress ( "مشکلی در ارسال اطلاعات به وجود آمده است" );
               app.t ( getString ( R.string.error ) , app.ToastType.ERROR );
          }
