package contagiouscode.mirsengar.rcandroid.adapter;
...
public
class CommutingAdapter extends RecyclerView.Adapter < CommutingAdapter.MyViewHolder > {

List < CommutingObject > list;
Activity                 activity;

public
CommutingAdapter ( List < CommutingObject > list , Activity activity ) {
     this.list     = list;
...
MyViewHolder onCreateViewHolder ( @NonNull ViewGroup parent , int viewType ) {
     View view = LayoutInflater.from ( activity ).inflate ( R.layout.row_commuting , parent ,
                                                            false );
...
void onBindViewHolder ( @NonNull MyViewHolder holder , int position ) {
     ...
     holder.commutingType.setTextColor (
               activity.getResources ( ).getColor (
                         people.getType ( ).equals ( "in" ) ? R.color.greenLevel1 :
                         R.color.redLevel1
                                                  )
                                       );
